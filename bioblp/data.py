...
def create_random_splits(triples: pd.DataFrame, train_ratio: float, valid_ratio: float, test_ratio: float):
    ...
    train, valid, test = triples_factory.split([train_ratio, valid_ratio, test_ratio], random_state=2021)

    train_triples = pd.DataFrame(train.triples, columns=[COL_SOURCE, COL_EDGE, COL_TARGET])
    valid_triples = pd.DataFrame(valid.triples, columns=[COL_SOURCE, COL_EDGE, COL_TARGET])
    test_triples = pd.DataFrame(test.triples, columns=[COL_SOURCE, COL_EDGE, COL_TARGET])
...
def save_splits(train_df, test_df, valid_df, dataset_name, out_dir):
    ...
    train.to_csv(out_dir.joinpath(f"{dataset_name}-train.tsv"), sep='\t', index=None)
    test.to_csv(out_dir.joinpath(f"{dataset_name}-test.tsv"), sep='\t', index=None)
    valid.to_csv(out_dir.joinpath(f"{dataset_name}-valid.tsv"), sep='\t', index=None)
...
def load_splits(dataset: str, data_path: str, dev_sample=False) -> (TriplesFactory, TriplesFactory, TriplesFactory):
    ...
    training = TriplesFactory.from_labeled_triples(train_df[[COL_SOURCE, COL_EDGE, COL_TARGET]].values)
    valid = TriplesFactory.from_labeled_triples(
        valid_df[[COL_SOURCE, COL_EDGE, COL_TARGET]].values, entity_to_id=training.entity_to_id,
        relation_to_id=training.relation_to_id)
    test = TriplesFactory.from_labeled_triples(
        test_df[[COL_SOURCE, COL_EDGE, COL_TARGET]].values, entity_to_id=training.entity_to_id,
        relation_to_id=training.relation_to_id)
