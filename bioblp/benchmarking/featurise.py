    ...
    def encode(self, x: Tensor, missing_value: str = "avg") -> Tuple[Tensor, Tensor]:
        ...
        for idx, x_i in tqdm(enumerate(x), total=len(x)):
            if x_i in self.entity_to_id:
                ...
                embs.append(self.impute_missing_value(x_i, missing_value=missing_value))
...
class NoiseEncoder(EntityEncoder):
    ...
    def from_entities(cls, entities, random_seed: int = 42, dim=128, emb_range=(-1, 1)):
        ...
        embs = get_random_tensor(shape=emb_shape, random_seed=random_seed, emb_range=emb_range)
...
class EntityPairEncoder():
    ...
    def encode(self, x, missing_value: str = "random", transform="concat") -> Tensor:
        ...
        left_enc, left_mask = self.left_encoder.encode(left, missing_value=missing_value)
        right_enc, right_mask = self.right_encoder.encode(right, missing_value=missing_value)
...
def RandomNoisePairEncoder(entities) -> EntityPairEncoder:
    ...
    pair_encoder = EntityPairEncoder(left_encoder=noise_encoder, right_encoder=noise_encoder)
    return pair_encoder
...
def StructuralPairEncoder(protein_path, mol_path) -> EntityPairEncoder:
    ...
    # prepare noisy embeddings
    noise_encoder = NoiseEncoder.from_entities(mol_path)
    ...
    pair_encoder = EntityPairEncoder(left_encoder=noise_encoder, right_encoder=prot_encoder)
    return pair_encoder


def build_features(bm_file: str, proteins, **kwargs):

    # load benchmark data
    # here entities are labels
    logger.info(bm_file, proteins)
    bm_df = pd.read_csv(bm_file, sep='\t', names=[COL_SOURCE, COL_EDGE, COL_TARGET, "label"])
    ...
    # ## pairs
    structural_encoder = StructuralPairEncoder(protein_path=proteins_path, mol_path=all_entities)
    ...
    pairs_enc, pairs_mask = structural_encoder.encode(pairs, transform="concat")
